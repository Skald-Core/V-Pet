...
void Mascote::recuperarEnergiaEIdade() {
    energia = 5;
    saciedade = std::max(0, saciedade - 1); // Acorda com um pouco de fome
    envelhecer();
}

void Mascote::alimentar(const ItemComida& comida, std::wstring& msgLog) {
    if (estaDormindo) {
        msgLog = nome + L" está dormindo e não pode comer agora!";
        return;
    }
    gastarEnergia();
    if (comida.getNome() == getComidaCerta()) {
        ...
        saude = std::max(0, saude - 1); 
        msgLog = nome + L" não gosta de " + comida.getNome() + L" e passou mal!";
    }
}

void Mascote::brincar(std::wstring& msgLog) {
    if (estaDormindo) {
        msgLog = nome + L" está dormindo e não quer brincar!";
        return;
    }
    gastarEnergia();
    if (energia == 0 || saciedade == 0) {
        ...
        saciedade = std::max(0, saciedade - 1);
        if (rand() % 3 == 0) {
            energia = std::max(0, energia - 2);
            msgLog = nome + L" gastou muita energia brincando!";
        } else {
